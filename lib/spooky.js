...
process.on('exit', function (code, signal) {
    _.each(Spooky._servers, function (server) {
        server.kill();
    });
});
...
Spooky.listen = function (options, callback) {
    ...
    if (this._servers[options.port]) {
        throw new Error('Already running a server on port ' + options.port);
    }

    var server = spawn('casperjs', args);
    server.on('exit', function (code, signal) {
        var e;
        if (code) {
            e = new Error('Server terminated with non-zero exit code ' + code);
            e.details = {
                code: code,
                signal: signal
            };
...
Spooky.unlisten = function (port) {
    this._servers[port].kill();
