...
function Spooky(options, callback) {
    ...
    // If a file: { path: "file_name.js", options: {...} } option is provided
    // then we wrap the callback to automatically execute the CasperJS file
    // on load (or emit an error upon failure).
    if (options.file) {
        ...
        callback = (function(err) {
            ...
            // Otherwise read in the specified file
                ...
                fs.readFile(options.file.path, (function(err, data) {
                    ...
                    // Execute the contents of the file, passing in any options
                    // that the user may have specified
                    } else {
                        this.exec(data.toString(), options.file.options);
                    }
                }).bind(this));
            }
        }).bind(this);
...
Spooky.prototype._spawnChild = function () {
    ...
    child = this._child = spawn(options.command, args);
